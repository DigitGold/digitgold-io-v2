export const fr = {
  meta: {
    title: "DigitGold | L'or du futur, frappé sur la blockchain",
    description:
      "Des NFTs représentant de l'or physique, stocké en Europe, audité et vérifiable sur la blockchain. Découvrez la Réserve DigitGold.",
  },
  nav: {
    story: "Notre Histoire",
    trust: "Pourquoi Nous Faire Confiance",
    reserve: "Accéder à la Réserve",
  },
  hero: {
    title:
      "Et si votre prochain NFT représentait 2 grammes d'or réel, stocké en Europe, vérifiable sur la blockchain ?",
    subtitle:
      "Une nouvelle approche du patrimoine à la croisée de l'investissement et du numérique.",
    cta: {
      primary: "Découvrir la Réserve",
      secondary: "Notre Histoire",
    },
  },
  story: {
    title: {
      first: "Notre Histoire",
      highlight: ""
    },
    subtitle:
      "Une vision qui transforme la façon dont nous percevons la valeur et le patrimoine numérique",
    blocks: {
      idea: {
        title: "La naissance de l'idée",
        description:
          "L'or a traversé les siècles comme une valeur refuge incontestable. Nous avons imaginé comment faire entrer ce métal précieux dans l'ère du Web3 tout en conservant ses qualités fondamentales : la rareté, la permanence et la confiance.",
      },
      reserve: {
        title: "La Réserve DigitGold",
        description:
          "5000 NFTs uniques, chacun représentant précisément 2 grammes d'or physique stockés dans nos coffres européens sécurisés. Une collection fondatrice, limitée et auditée régulièrement.",
      },
      mechanism: {
        title: "Le mécanisme",
        description:
          "Chaque NFT est lié à son équivalent physique. Un système de burn permettra à 80% des tokens d'être progressivement détruits sur 5 ans, renforçant la rareté et la valeur des 20% restants.",
      },
      legacy: {
        title: "L'héritage",
        description:
          "Au-delà de la possession, nous créons un nouvel écosystème patrimonial. Fusionnez vos NFTs pour créer des pièces uniques sur mesure, ou transformez-les en or physique après un staking de 12 mois.",
      },
    },
  },
  trust: {
    title: "Pourquoi faire confiance à DigitGold ?",
    subtitle:
      "Nous avons construit notre protocole sur des principes inébranlables de sécurité, transparence et confiance",
    features: {
      storage: {
        title: "Stocké en Europe",
        description:
          "Votre or est stocké dans des installations hautement sécurisées en Europe, avec des normes de sécurité militaires et une assurance complète.",
      },
      audit: {
        title: "Audit semestriel par un tiers",
        description:
          "Des auditeurs indépendants vérifient l'existence et la quantité de l'or physique tous les six mois. Les rapports sont rendus publics et accessibles à tous.",
      },
      certificates: {
        title: "Certificats & Blockchain Proofs",
        description:
          "Chaque NFT est associé à un certificat numérique signé et une traçabilité complète via la blockchain Polygon.",
        details: [
          "Certificat numérique unique pour chaque NFT",
          "Lien IPFS du certificat stocké publiquement",
          "Hash de métadonnées ancré sur la blockchain",
          "Audits indépendants 2x/an, résultats accessibles",
          "Smart contract open source vérifié",
        ],
      },
      blockchain: {
        title: "Blockchain vérifiable",
        description:
          "Chaque NFT est lié à un smart contract sur Polygon, offrant transparence et immuabilité. La propriété est inscrite de manière permanente et vérifiable.",
      },
    },
    quote:
      "DigitGold n'est ni une spéculation, ni une série d'art PFP. C'est une passerelle entre valeur réelle et innovation, pensée pour durer.",
  },
  transformation: {
    title: "Mécanismes de Conversion",
        subtitle:
      "Grâce à ces mécanismes innovants, votre NFT devient bien plus qu'un simple actif numérique",
    features: {
      burn: {
        title: "Programme de Burn",
        description:
          "80% des NFTs seront progressivement brûlés sur 5 ans, augmentant mécaniquement la rareté et la valeur potentielle des 20% restants. Chaque opération de burn est publique, traçable et irréversible.",
        note:
          "Une stratégie de rareté programmée qui renforce la valeur dans le temps.",
      },
      staking: {
        title: "Staking Obligatoire",
        description:
          "Pour échanger votre NFT contre son équivalent en or physique, un staking minimum de 12 mois est requis. Cette règle crée une pression temporelle favorable et récompense les détenteurs de long terme.",
        note:
          "Un mécanisme d'engagement qui favorise une vision long terme.",
      },
      fusion: {
        title: "Fusion Sur-Mesure",
        description:
          "Fusionnez plusieurs NFTs pour créer une pièce exclusive de plus grande valeur. Par exemple, combinez 3 NFTs de 2g pour obtenir un NFT unique de 6g avec un design exclusif frappé à la demande.",
        note:
          "Une personnalisation exclusive qui augmente la valeur émotionnelle.",
      },
    },
  },
  roadmap: {
    title: "Notre",
    highlight: "Vision Long Terme",
    subtitle: "Une feuille de route ambitieuse pour révolutionner l'investissement dans l'or",
    q2: {
      title: "Lancement initial",
      points: [
        "Lancement de DigitGold Collection 1 (5000 NFTs de 2g)",
        "Déploiement sur Polygon avec paiement en $MATIC",
        "Stockage sécurisé en Europe (hors Suisse)",
        "Audit de la réserve semestriel par organisme externe",
        "Intégration d'une galerie immersive dédiée"
      ]
    },
    q3: {
      title: "Croissance organique & communauté",
      points: [
        "Mise en place du lead magnet (capture d'emails)",
        "Lancement d'un mini-site éducatif sur l'or et les NFT",
        "Influenceurs ciblés Web3 / Finance / Précieux",
        "Préparation du staking des NFTs"
      ]
    },
    q4: {
      title: "Fonction Staking & fusion exclusive",
      points: [
        "Déblocage du staking (1 an minimum) → droit au burn",
        "Lancement du système de burn/fusion pour upgrade",
        "Premier NFT \"spécial\" pour les stackeurs fidèles"
      ]
    },
    2026: {
      title: "Nouvelle collection & infrastructure",
      points: [
        "Création d'une Collection 2 avec variation de grammage",
        "Intégration avec des wallets mobiles pour simplifier l'accès",
        "Déploiement d'un tableau de bord utilisateur"
      ]
    },
    2027: {
      title: "Marketplace & institutionnels",
      points: [
        "Lancement d'un mini-market secondaire natif (KYC requis)",
        "Ouverture de la plateforme B2B (revendeurs, family offices)",
        "Début d'une campagne d'évangélisation institutionnelle"
      ]
    },
    2028: {
      title: "Expansion physique & conformité",
      points: [
        "Projet de carte physique de garde ou d'accès",
        "Alignement avec réglementations européennes à venir",
        "Roadshow mondial + Conférence annuelle DigitGold",
        "Objectif : Or tokenisé = nouvelle norme d'épargne alternative"
      ]
    }
  },
  footer: {
    about:
      "L'or du futur, frappé sur la blockchain. Une nouvelle approche du patrimoine digital.",
    quickLinks: {
      title: "Liens Rapides",
      story: "Notre Histoire",
      trust: "Garanties et Transparence",
      opensea: "OpenSea",
    },
    trust: {
      title: "Confiance",
      audit: "Audits semestriels par un tiers",
      storage: "Stockage sécurisé en Europe",
      blockchain: "Blockchain Polygon vérifiable",
    },
    contact: {
      title: "Contact",
      email: "contact@digitgold.io",
    },
    legal: {
      copyright: "© 2024 DigitGold. Tous droits réservés.",
      terms: "Mentions Légales",
      privacy: "Politique de Confidentialité",
      conditions: "CGV",
    },
  },
  cta: {
    title: {
      part1: "Prêt à rejoindre",
      highlight: "La Réserve"
    },
    description: "Soyez parmi les premiers à investir dans la première collection DigitGold : 5000 NFTs, chacun représentant 2 grammes d'or physique, stockés en Europe.",
    button: {
      reserve: "Accéder à la Réserve",
      opensea: "Voir sur OpenSea"
    }
  },  
  modal: {
    title: "Recevez votre accès à la Réserve DigitGold",
    subtitle:
      "Soyez parmi les premiers à accéder à notre collection exclusive avant tout le monde.",
    fields: {
      email: "Email",
      name: "Nom (optionnel)",
    },
    placeholders: {
      email: "votre@email.com",
      name: "Votre nom",
    },
    submit: "Recevoir mon accès",
    disclaimer:
      "En vous inscrivant, vous acceptez de recevoir des emails de DigitGold. Vous pourrez vous désinscrire à tout moment.",
    success: {
      title: "Merci pour votre inscription !",
      description:
        "Vous recevrez bientôt votre accès exclusif à la Réserve DigitGold et toutes les informations importantes concernant notre lancement.",
      close: "Fermer",
    },
    error: {
      emptyEmail: "Veuillez saisir votre adresse email",
      invalidEmail: "Veuillez saisir une adresse email valide",
    },
  },
};
